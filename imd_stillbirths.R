...
# Perform Kolmogorov-Smirnov test to check whether stillbirth variable is normally distributed or not
ks.test(
	imdStillBirth$Stillbirths,
	"pnorm"
)

# Calculate rank for column IMD and Stillbirths
imdStillBirth$IMD_rank <- rank(imdStillBirth$IMD)
imdStillBirth$Stillbirths_rank <- rank(imdStillBirth$Stillbirths)

# Perform Kendall' tau correlation test
cor.test(imdStillBirth$IMD, imdStillBirth$Stillbirths, method = "kendall")

# Generate scatter plot with regression line and correlation coefficient
ggscatter(
	imdStillBirth, x = "IMD", y = "Stillbirths",
	add = "reg.line", conf.int = TRUE,
	cor.coef = TRUE, cor.method = "kendall", cor.coef.size = 8,
	title = "Correlation between the IMD and number of stillbirths",
	xlab = "Index of Multiple Deprivation",
	ylab = "Number of Stillbirths"
